...
// Add creates a new TargetPool Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New("targetpool-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource TargetPool
	err = c.Watch(&source.Kind{Type: &computev1.TargetPool{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// TODO(user): Modify this to be the types you create that are owned by the primary resource
	// Watch for changes to secondary resource Pods and requeue the owner TargetPool
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
		OwnerType:    &computev1.TargetPool{},
	})
	if err != nil {
		return err
	}
...
var _ reconcile.Reconciler = &ReconcileTargetPool{}

// ReconcileTargetPool reconciles a TargetPool object
type ReconcileTargetPool struct {
	...
	spec            *gceCompute.TargetPool
	k8sObject       *computev1.TargetPool
}

// Reconcile reads that state of the cluster for a TargetPool object and makes changes based on the state read
// and what is in the TargetPool.Spec
// TODO(user): Modify this Reconcile function to implement your Controller logic.  This example creates
// a Pod as an example
// Note:
// The Controller will requeue the Request to be processed again if the returned error is non-nil or
// Result.Requeue is true, otherwise upon completion it will remove the work from the queue.
func (r *ReconcileTargetPool) Reconcile(request reconcile.Request) (reconcile.Result, error) {
