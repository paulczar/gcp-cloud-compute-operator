...
// Add creates a new ManagedZone Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New("mangedzone-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource ManagedZone
	err = c.Watch(&source.Kind{Type: &dnsv1.ManagedZone{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// TODO(user): Modify this to be the types you create that are owned by the primary resource
	// Watch for changes to secondary resource Pods and requeue the owner ManagedZone
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
		OwnerType:    &dnsv1.ManagedZone{},
	})
	if err != nil {
		return err
	}
...
var _ reconcile.Reconciler = &ReconcileManagedZone{}

// ReconcileManagedZone reconciles a ManagedZone object
type ReconcileManagedZone struct {
	...
	spec            *gceDNS.ManagedZone
	k8sObject       *dnsv1.ManagedZone
}

// Reconcile reads that state of the cluster for a ManagedZone object and makes changes based on the state read
// and what is in the ManagedZone.Spec
// TODO(user): Modify this Reconcile function to implement your Controller logic.  This example creates
// a Pod as an example
// Note:
// The Controller will requeue the Request to be processed again if the returned error is non-nil or
// Result.Requeue is true, otherwise upon completion it will remove the work from the queue.
func (r *ReconcileManagedZone) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	var finalizer = utils.Finalizer
	log.Printf("Reconciling mangedzone %s/%s\n", request.Namespace, request.Name)
	...
	if gceObject != nil {
		...
		if gceObject.CreationTime != "" {
			r.k8sObject.Status.Status = "READY"
		}
		r.k8sObject.Status.NameServers = gceObject.NameServers
