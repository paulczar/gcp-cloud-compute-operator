...
// Add creates a new Firewall Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New("firewall-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource Firewall
	err = c.Watch(&source.Kind{Type: &computev1.Firewall{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// TODO(user): Modify this to be the types you create that are owned by the primary resource
	// Watch for changes to secondary resource Pods and requeue the owner Firewall
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
		OwnerType:    &computev1.Firewall{},
	})
	if err != nil {
		return err
	}
...
var _ reconcile.Reconciler = &ReconcileFirewall{}

// ReconcileFirewall reconciles a Firewall object
type ReconcileFirewall struct {
	...
	spec            *gceCompute.Firewall
	k8sObject       *computev1.Firewall
}

// Reconcile reads that state of the cluster for a Firewall object and makes changes based on the state read
// and what is in the Firewall.Spec
// TODO(user): Modify this Reconcile function to implement your Controller logic.  This example creates
// a Pod as an example
// Note:
// The Controller will requeue the Request to be processed again if the returned error is non-nil or
// Result.Requeue is true, otherwise upon completion it will remove the work from the queue.
func (r *ReconcileFirewall) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	log.Printf("Reconciling Firewall %s/%s\n", request.Namespace, request.Name)
