...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &computev1.Instance{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func (r *ReconcileInstance) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	log.Printf("Reconciling Instance %s/%s\n", request.Namespace, request.Name)
	...
	if gceObject != nil {
		...
		if r.k8sObject.Status.Status == "RUNNING" {
		...
		r.k8sObject.Status.CpuPlatform = gceObject.CpuPlatform
		r.k8sObject.Status.StartRestricted = gceObject.StartRestricted
		r.k8sObject.Status.StatusMessage = gceObject.StatusMessage
